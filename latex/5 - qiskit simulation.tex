...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.48\textwidth]{plots/qiskit/qcs/charge_qc.png}
...
\begin{figure*}[tb!]
    ...
    \begin{subfigure}{0.33\textwidth}
        \includegraphics[width=\textwidth]{plots/qiskit/Alice/N1_energy_vs_J.png}
        \caption{Energy, \( N = 1 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.33\textwidth}
        \includegraphics[width=\textwidth]{plots/qiskit/Alice/N2_energy_vs_J.png}
        \caption{Energy, \( N = 2 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.33\textwidth}
        \includegraphics[width=\textwidth]{plots/qiskit/Alice/N3_energy_vs_J.png}
        \caption{Energy, \( N = 3 \)}
    \end{subfigure}
    ...
    \begin{subfigure}{0.33\textwidth}
        \includegraphics[width=\textwidth]{plots/qiskit/Alice/N1_charge_vs_J.png}
        \caption{Charge, \( N = 1 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.33\textwidth}
        \includegraphics[width=\textwidth]{plots/qiskit/Alice/N2_charge_vs_J.png}
        \caption{Charge, \( N = 2 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.33\textwidth}
        \includegraphics[width=\textwidth]{plots/qiskit/Alice/N3_charge_vs_J.png}
        \caption{Charge, \( N = 3 \)}
    \end{subfigure}
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/qiskit/NearestNeighbors/X/N2_energy_vs_J.png}
        \caption{Energy, \( N = 2 \), \( \sigma_A = X_0 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/qiskit/NearestNeighbors/X/N3_energy_vs_J.png}
        \caption{Energy, \( N = 3 \), \( \sigma_A = X_0 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/qiskit/NearestNeighbors/Y/N2_energy_vs_J.png}
        \caption{Energy, \( N = 2 \), \( \sigma_A = Y_0 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/qiskit/NearestNeighbors/Y/N3_energy_vs_J.png}
    ...
    \begin{subfigure}{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/qiskit/NearestNeighbors/X/N2_charge_vs_J.png}
        \caption{Charge, \( N = 2 \), \( \sigma_A = X_0 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/qiskit/NearestNeighbors/X/N3_charge_vs_J.png}
        \caption{Charge, \( N = 3 \), \( \sigma_A = X_0 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/qiskit/NearestNeighbors/Y/N2_charge_vs_J.png}
        \caption{Charge, \( N = 2 \), \( \sigma_A = Y_0 \)}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/qiskit/NearestNeighbors/Y/N3_charge_vs_J.png}
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{plots/qiskit/real hw/real_hw_charge_vs_J.png}
        \caption{Charge teleportation}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{plots/qiskit/real hw/real_hw_energy_vs_J.png}
        \caption{Energy teleportation}
    \end{subfigure}
    \caption{Experimental protocol results obtained from real quantum hardware for \( N = 1 \), using Alice’s \( \sigma_A = X_0 \) basis. Each point represents an independent execution for a fixed value of coupling \( J \).\newline\textbf{\textcolor{BrickRed}{TODO: Update to new results with error bars after running the protocol}}}
    \label{fig:real_hw_results}
\end{figure*}

To complete the analysis, we also implement the teleportation protocol on an actual quantum processor via the IBM Q backend. The experiment was conducted for the minimal configuration with \( N = 1 \) using Alice's basis \( \sigma_A = X_0 \), which corresponds to both the \( H^{(1)} \) and \( H^{(2)} \) models in this simple setting. The charge and energy observables were measured separately using the circuits introduced earlier.

Although the data points are fewer than in the simulated case—due to the significant overhead and resource demand of real hardware runs—the results clearly indicate that the protocol operates as expected even in the presence of device noise. For both energy and charge, the results exhibit the characteristic split between the two classical outcomes (\( c \) or \( c \oplus 1 \)), affirming the correctness of the teleportation mechanism.

The robustness of charge teleportation is particularly evident here, aligning with our earlier observations from simulations. This experiment provides practical validation that observable teleportation is feasible on real quantum hardware using only local operations and classical communication, without requiring full quantum state teleportation or entanglement swapping.

\subsection{Summary: Insights from Qiskit Simulations}

The results presented in this section highlight both the promise and practical limitations of observable teleportation protocols when implemented in real or simulated quantum systems:

\begin{itemize}
    \item \textbf{Charge vs. Energy Teleportation:} Across all experiments, charge teleportation demonstrates higher stability, cleaner signal separation, and reduced statistical noise. This superior statistical clarity, stemming from its single-operator measurement, is a key practical advantage for QKD. It implies that a charge-based protocol would require fewer experimental shots to establish a key bit with high confidence, making it more efficient and scalable with system size \( N \) on near-term quantum devices.

    \item \textbf{Hardware Scalability:} While teleportation works well for small \( N \), scaling to larger systems introduces challenges. In the nearest-neighbor model, indirect interaction between Alice and Bob weakens the effective signal and increases the number of shots required for convergence. This highlights the trade-off between physical model realism and practical observability.

    \item \textbf{Measurement-Induced Noise:} For energy teleportation, the need to separately estimate components of Bob’s Hamiltonian (due to non-commuting terms) introduces statistical inconsistencies. This is exacerbated in longer chains where signal propagation is more fragile.

    \item \textbf{Basis Sensitivity:} Simulations with different measurement bases for Alice (\( \sigma_A = X_0 \) vs. \( \sigma_A = Y_0 \)) show that the protocol’s output and robustness are basis-dependent. The X basis typically exhibits more resilience to noise, making it more suitable for experimental implementations.

    \item \textbf{Practical Feasibility:} Most importantly, all results consistently support the theoretical predictions of the QET protocol. The Qiskit simulations reproduce the numerical results with high fidelity, and the real hardware experiment—despite lower resolution—demonstrates that the protocol is experimentally realizable even under present-day quantum hardware constraints.
\end{itemize}

Together, these insights underscore the power of observable teleportation as a physically meaningful and technically feasible protocol for quantum information processing.

A key observation from the simulations is the difference in statistical stability between the charge and energy teleportation protocols. The increased fluctuations in the energy plots are not merely noise but a direct and predictable consequence of the measurement process itself. As derived in Appendix~\ref{appendix:stats-analysis}, Bob's local Hamiltonian $H_B$ consists of non-commuting terms that must be measured in separate experiments. The total statistical error on $\langle H_B \rangle$ is therefore a sum in quadrature of the uncertainties from each independent measurement, a process known as error propagation. This inherently leads to a larger final uncertainty compared to the charge observable $Q_B$, which is determined from a single experimental setup.

Furthermore, the degradation of the signal for larger systems (e.g., $N=3, 4$ in the nearest-neighbor model) can be understood through the Signal-to-Noise Ratio (SNR). As the distance between Alice and Bob increases, the teleported signal $\abs{\langle O \rangle}$ weakens, while the intrinsic variance of a single-shot measurement remains roughly constant. Consequently, the SEM becomes comparable to the signal itself, requiring an exponentially larger number of shots to resolve the expectation value from the statistical noise floor.

These results highlight the statistical robustness of the charge teleportation protocol. Its reliance on a single measurement basis and a strong, consistent signal across different coupling regimes makes it a more resilient and practical candidate for applications like QKD, particularly in the presence of finite-shot noise characteristic of near-term quantum devices.
