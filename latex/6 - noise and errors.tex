...
\begin{figure}[ht]
  ...
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/numerical/Alice_charge_vs_p_class_comm_err.png}
    \caption{\(H^{(1)}\) (Star-Coupling interaction)}
  \end{subfigure}
  \vspace{0.5em}
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/numerical/NN_X_charge_vs_p_class_comm_err.png}
    \caption{\(H^{(2)}\) (Nearest-Neighbors interaction), \(\sigma_A=X_0\)}
  \end{subfigure}
  \vspace{0.5em}
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/numerical/NN_Y_charge_vs_p_class_comm_err.png}
    \caption{\(H^{(2)}\) (Nearest-Neighbors interaction), \(\sigma_A=Y_0\)}
  \end{subfigure}
...
From Figure~\ref{fig:classical-main} one can see that error flipping the classical bit communicated from Alice to Bob changes the extracted value sign at Bob's site at high probabilities. This is the expected result since by the protocol's design and from simulations of the protocol in previous sections we showed that flipping this bit inverts the sign measured by Bob. Comparing to energy from the equivalence simulation in~\cite{QKDbyQET} (see also Figures~\ref{fig:classical_err_alice_ham},~\ref{fig:classical_err_nn_ham_x} and~\ref{fig:classical_err_nn_ham_y} in Appendix), the charge is somewhat more robust to such error where the sign flips around \(p_{err} \approx  0.32\) instead of \(p_{err} \approx  0.25\) for energy.
...
\begin{figure}[ht!]
  ...
  \begin{subfigure}{0.35\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/numerical/N2_charge_vs_p_excited_mixture_error.png}
    \caption{Numerical, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
  \vspace{0.5em}
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/qiskit/N2_charge_vs_p_excited_mixture_error.png}
    \caption{Qiskit, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
...
Figure~\ref{fig:mixture-main} reveals a critical performance difference between energy and charge teleportation. While both signals degrade as the mixture probability $p$ increases, the energy signal frequently crosses zero and becomes positive at moderate error levels ($p \approx 0.15-0.25$) \cite{QKDbyQET}. In contrast, the charge signal often remains negative even at high error probabilities, merely decreasing in magnitude. This superior robustness of charge stems from the nature of the observables. Low-lying excitations can significantly alter the local energy expectation value $\langle H_B \rangle$, contributing a large positive offset term ($\xi_H$) that overwhelms the negative teleported signal from the ground state. The charge observable $Q_B$, which is tied to local spin parity, is less sensitive to such energy shifts. The correlations responsible for charge teleportation ($\eta_Q$) often retain their sign across different low-energy manifolds, preserving the integrity of the key bit.
...
\begin{figure}[ht!]
  ...
  \begin{subfigure}{0.35\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/numerical/N2_charge_vs_p_excited_superposition_error.png}
    \caption{Numerical, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
  \vspace{0.5em}
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/qiskit/N2_charge_vs_p_excited_superposition_error.png}
    \caption{Qiskit, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
...
A coherent error creating a superposition $|\psi\rangle = \sqrt{1-p}|\psi_{gs}\rangle + e^{i\alpha}\sqrt{p}|\psi_{excited}\rangle$ introduces interference terms into the expectation values of $\xi$ and $\eta$. This leads to a non-linear dependence on the error probability $p$, as shown in Figure~\ref{fig:superposition-main}. Similar to the classical mixture, the energy protocol is more susceptible to sign-flips than the charge protocol. The phase sensitivity inherent in the superposition can destructively interfere with the commutator term $\eta$, which is the engine of teleportation, and this effect appears more pronounced for the energy observable. The Qiskit simulations again highlight the increased statistical variance, making it difficult to distinguish the true non-linear signal from finite-shot noise.
...
\begin{figure}[ht!]
  ...
  \begin{subfigure}{0.35\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/numerical/N2_charge_vs_p_bitflip_error.png}
    \caption{Numerical, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
  \vspace{0.5em}
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/qiskit/N2_charge_vs_p_bitflip_error.png}
    \caption{Qiskit, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
...
\begin{figure}[ht!]
  ...
  \begin{subfigure}{0.35\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/numerical/N2_charge_vs_p_alice_phaseflip_error.png}
    \caption{Numerical, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
  \vspace{0.5em}
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/qiskit/N2_charge_vs_p_alice_phaseflip_error.png}
    \caption{Qiskit, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
  
  \caption{Phase-flip error on Alice's qubit for the $H^{(2)}$ model ($N=2, \sigma_A=X_0$). The error attenuates the signals by decohering Alice's measurement basis, but sign-flips are not prominent at low error probabilities.}
  \label{fig:phaseflip-alice-main}
\end{figure}
...
\begin{figure}[ht!]
  ...
  \begin{subfigure}{0.35\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/numerical/N2_charge_vs_p_bob_phaseflip_error.png}
    \caption{Numerical, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
  \vspace{0.5em}
  \begin{subfigure}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{plots/errors/qiskit/N2_charge_vs_p_bob_phaseflip_error.png}
    \caption{Qiskit, \(H^{(2)}\), \(N{=}2\), \(\sigma_A=X_0\)}
  \end{subfigure}
  
  \caption{Phase-flip error on Bob's qubit for the $H^{(2)}$ model ($N=2, \sigma_A=X_0$). This error is more detrimental than a flip on Alice's side, causing a rapid decay by inverting Bob's conditional rotation.}
  \label{fig:phaseflip-bob-main}
\end{figure}

% This table belongs to the next subsection, it is here for final PDF organization to appear in a better place.
\begin{table*}[t]
\small
\centering
...
\textbf{\textcolor{BrickRed}{TODO: Review and add references to energy graphs in appendix and QKD by QET paper since they are moved out of main section.}}
